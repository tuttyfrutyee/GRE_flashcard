items = {
    #Intermediate Level 1 Continues

    "modicum" : "a small amount",

    "apogee" : "highest point",

    #Intermediate Level 2

    "tyro" : "beginner",

    "myriad" : "large number",

    "desecrate" : "ruin something holy",

    "conducive" : "helpful",

    "derogative" : "insulting",

    "loath" : "reluctant",

    "placid" : "peaceful, tranquil",

    "despot" : "tyrant",

    "expunge" : "erase",

    "obstinate" : "stubborn",

    "maverick" : "radical thinker",

    "morose" : "gloomy",

    "evanescent" : "short-lived",

    "ossify" : "harden",

    "puerile" : "childish",

    #Intermediate Level 3

    "diatribe" : "bitter verbal attack",

    "dog" : "pursue",

    "debunk" : "prove to be false, disabuse",

    "ravenous" : "very hungry",

    "inchoate" : "not fully formed",

    "debase" : "reduce the value",

    "conciliate" : "make peace with",

    "unnerve" : "make one lose confidence",

    "unseemly" : "improper",

    "admonish" : "warn, reprove",

    "pariah" : "outcast",

    "tact" : "proper social behaviour [display of tact]",

    "antic" : "grotesque",

    "telltale" : "revealing [telltale signs]",

    "spite" : "malice",

    "avarice" : "greed",

    "ebullient" : "full of confidence, energy and good humour",

    "nuance" : "subtlety",

    "lachrymose" : "weepy",

    


}




























































